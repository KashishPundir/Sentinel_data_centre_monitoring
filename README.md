...
3. Start the API: `uvicorn api.app:app --host 0.0.0.0 --port 8000`. It starts
   and supervises the telemetry consumer used by the dashboard.
4. Start the dashboard: `streamlit run dashboard/app.py`
5. Open the dashboard or `/docs`, `/health/live`, `/health/ready`, then start the replay through
   the simulation endpoints.

`/monitoring/system-status` reports the inference-worker state and its last
startup error, so a Kafka/model problem is visible in the dashboard instead
of appearing as an unexplained lack of telemetry updates.
